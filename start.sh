...
# ============================================================================
# shredcore-scraper-bot start.sh
# Interactive setup and launch script for the Solana scraper bot.
# ============================================================================
...
# Set a raw TOML array (for complex inline tables)
# Usage: set_toml_raw_array KEY "[{ ... }, { ... }]"
set_toml_raw_array() {
    local key="$1"
    local raw_content="$2"
    
    # Use awk to replace the array (handles multi-line arrays)
    awk -v key="$key" -v content="$raw_content" '
    BEGIN { in_array = 0; found = 0 }
    {
        if (match($0, "^[[:space:]]*" key "[[:space:]]*=")) {
            found = 1
...
configure_discord() {
    ...
    if ! prompt_yes_no "Enable Discord scraping?"; then
        set_toml_bool "ENABLE_DISCORD" "false"
        return
    fi
    
    set_toml_bool "ENABLE_DISCORD" "true"
    
    # Discord User Token (required)
    local discord_token
    prompt_required "Enter Discord user token" discord_token
    set_toml_string "DISCORD_USER_TOKEN" "$discord_token"
    
    # Discord Sources
    echo ""
    echo "Add Discord guild/channel combinations to monitor."
    echo "You can add multiple sources. Press Enter with empty guild ID to finish."
    
    local discord_sources=""
    local source_count=0
    
    while true; do
        echo ""
        local guild_id
        read -rp "Guild ID (or press Enter to finish): " guild_id
        ...
        local channels
        read -rp "Channel IDs (comma-separated): " channels
        
        if [[ -z "$channels" ]]; then
            echo "At least one channel ID is required. Skipping this source."
            continue
        fi
        ...
        for chan in "${channel_list[@]}"; do
            ...
            if [[ -n "$chan" ]]; then
                if $first_chan; then
                    ...
                    channel_array="$channel_array, $chan"
                fi
            fi
        done
        
        # Build source object
        local source_obj="{ guild_id = $guild_id, channels = [$channel_array] }"
        
        if [[ -z "$discord_sources" ]]; then
            discord_sources="$source_obj"
        else
    ...
    if [[ $source_count -eq 0 ]]; then
        echo ""
        echo "WARNING: No Discord sources added. Discord scraping will be disabled until you add sources to config.toml"
        set_toml_raw_array "DISCORD_SOURCES" "[]"
    else
        set_toml_raw_array "DISCORD_SOURCES" "[$discord_sources]"
        echo ""
        echo "Added $source_count Discord source(s)."
    fi
}
...
configure_telegram() {
    ...
    if ! prompt_yes_no "Enable Telegram scraping?"; then
        set_toml_bool "ENABLE_TELEGRAM" "false"
        return
    fi
    
    set_toml_bool "ENABLE_TELEGRAM" "true"
    
    # Telegram API credentials
    local api_id
    prompt_required "Enter Telegram API ID" api_id
    set_toml_int "TELEGRAM_API_ID" "$api_id"
    
    local api_hash
    prompt_required "Enter Telegram API hash" api_hash
    set_toml_string "TELEGRAM_API_HASH" "$api_hash"
    
    local phone_number
    prompt_required "Enter phone number (with country code, e.g., +1234567890)" phone_number
    set_toml_string "TELEGRAM_PHONE_NUMBER" "$phone_number"
    
    # Telegram Sources
    echo ""
    echo "Enter Telegram source URLs (comma-separated)."
    echo "Supported formats:"
    echo "  - Public channel/group: https://t.me/gramme_rs"
    echo "  - Private invite link: https://t.me/+UGEkFgsh8785ZmVk"
    echo "  - Topic link: https://t.me/Freedomia_io/1"
    
    local sources
    read -rp "Telegram sources: " sources
    
    if [[ -z "$sources" ]]; then
        echo ""
        echo "WARNING: No Telegram sources added. Telegram scraping will be disabled until you add sources to config.toml"
        set_toml_raw_array "TELEGRAM_SOURCES" "[]"
    else
        # Build source array
        local telegram_sources=""
        ...
        for src in "${source_list[@]}"; do
            ...
            if [[ -n "$src" ]]; then
                local escaped_src
                escaped_src=$(escape_toml_string "$src")
                if $first; then
                    telegram_sources="{ url = \"$escaped_src\" }"
                    first=false
                else
                    telegram_sources="$telegram_sources, { url = \"$escaped_src\" }"
                fi
            fi
        done
        
        set_toml_raw_array "TELEGRAM_SOURCES" "[$telegram_sources]"
        echo "Telegram sources configured."
    fi
}

configure_x() {
    echo ""
    echo "--- X (Twitter) Configuration ---"
    
    if ! prompt_yes_no "Enable X (Twitter) scraping?"; then
        set_toml_bool "ENABLE_X" "false"
        return
    fi
    
    set_toml_bool "ENABLE_X" "true"
    
    # X Sources
    echo ""
    echo "Enter X usernames to monitor (comma-separated, with or without @)."
    
    local sources
    read -rp "X usernames: " sources
    
    if [[ -z "$sources" ]]; then
        echo ""
        echo "WARNING: No X sources added. X scraping will be disabled until you add sources to config.toml"
        set_toml_string_array "X_SOURCES" ""
    else
        set_toml_string_array "X_SOURCES" "$sources"
        echo "X sources configured."
    fi
}

configure_scraper_config() {
    echo ""
    ...
    echo ""
    echo "Scraper configuration complete!"
    echo "You can tune filters, keywords, and other settings in config.toml"
}
...
main() {
    echo "========================================"
    echo "  shredcore-scraper-bot Start Script"
    echo "========================================"
